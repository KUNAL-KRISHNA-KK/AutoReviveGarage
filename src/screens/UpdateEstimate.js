...
const UpdateEstimate = () => {
  ...
  const [imageArray, setImageArray] = useState(data?.Files ? data.Files : []);
  const [pickedImage, setPickedImage] = useState(null);
  const navigation = useNavigation();
  const [loading, setLoading] = useState(false);
  const [make, setMake] = useState('');
  const [makeId, setMakeId] = useState(data?.make_id);
  ...
  const [customer, setCustomer] = useState(
    data?.customer
      ? data?.customer[0]?.first_name + ' ' + data?.customer[0]?.last_name
      : '',
  );
  const [customerId, setCustomerId] = useState(data?.customer[0]?.id);
  ...
  const handleNext = () => {
    if (customer) {
      if (imageArray?.length > 0) {
        ...
        navigation.navigate('UpdateFullEstimate', { services, ApiData, data });
      }
    } else {
  ...
  const uploadPhoto = async () => {
    ...
    try {
      ...
      navigation.navigate('UpdateFullEstimate', {
        services,
        ApiData,
        photos,
        data,
