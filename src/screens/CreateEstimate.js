...
const CreateEstimate = () => {
  ...
  const validationSchema = yup.object().shape({
    customerF: yup.string().required('Customer is required'),
  ...
  const uploadPhoto = async () => {
    ...
    if (cameraPermission) {
      setLoading(true);
      try {
        const formData = convertFilesToFormData(imageArray);
        formData.append('module', 'Estimate');
        const res = await APISERVICES.uploadPhoto.post(formData);
        const photos = res?.data;
        if (res) {
          navigation.navigate('FullEstimate', { services, ApiData, photos });
        }
      } catch (err) {
        console.log(err);
      } finally {
  ...
  const handleNext = () => {
    if (customer) {
      if (imageArray.length > 0) {
        ...
        navigation.navigate('FullEstimate', { services, ApiData });
      }
    } else {
      CustomToast('info', 'Please select customer!');
    }
  };
      ...
      <KeyboardAwareScrollView>
            ...
            {imageArray?.map((item, index) => (
              <TouchableOpacity
                onPress={() => {
                  ...
                  navigation.navigate('ImageXL', { path: item.uri });
                }}
                style={{ marginLeft: 10 }}
                key={index}>
        ...
        <TouchableOpacity
          ...
          <TextInput
            ...
            style={{ marginLeft: 5, width: '90%', color: 'black', height: 100, height: 52 }}
            placeholderTextColor="#A6AFC8"
